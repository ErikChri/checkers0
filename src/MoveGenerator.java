...
public class MoveGenerator {
	...
	int move_value = 0;//add 1 for each capture
	...
	public MoveGenerator(HashMap<Point, Integer> testBoard, int color_value){
		...
		//for each piece find the possible moves and associated move value
		...
		//remove simple moves if capture move is possible
		remove_illegals();

	}

	private void generate_move(HashMap<Point, Integer> testBoard, Point point_to_move, boolean second_move){
		//get the possible moves for the test point
		next_move = new Move(testBoard, color_value, point_to_move, second_move);
		//store the moves and associated booelan in arrays
		...
		//for that piece add possible moves associated with the move value to the list
		for(int i=0; i<4; i++){
			if(move_direction_capture[i]  && (configuration[i].containsValue(1) || configuration[i].containsValue(11)) && (configuration[i].containsValue(-1) || configuration[i].containsValue(9))){ // && next_move.move_value>0){
				...
				generate_move(testBoard, point_moved_to[i], true); //if the move was a capture see if more captures are possible
			}
